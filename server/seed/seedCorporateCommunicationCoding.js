/**
 * seedCorporateCommunicationCoding.js
 * Seeds / cleans CodingProblem documents for all Corporate Communication topics.
 * Run with: node server/seed/seedCorporateCommunicationCoding.js
 */
...
const codingData = require("../data/subjects/corporateCommunicationCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Corporate Communication subject
  const subject = await Subject.findOne({ name: /^corporate communication$/i });
  if (!subject) {
    console.error("Corporate Communication subject not found");
    process.exit(1);
  }
