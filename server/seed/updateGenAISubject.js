...
const updateGenAISubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Generative AI subject update...");

    // 1. Find existing subject
    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Generative AI$/i }
    });
    ...
    if (!subjectDoc) {
      ...
      const genAiData = additionalSubjects.find(s => s.name.toLowerCase().includes("generative ai"));
      subjectDoc = await Subject.create(genAiData);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    // 2. Update subject's concepts list to match the 16 modules
    const concepts = genAiTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    const topicDocsToInsert = genAiTopics.map(t => ({
    ...
    for (const rawTopic of genAiTopics) {
    ...
    console.log("✅ Generative AI update completed successfully!");
    process.exit(0);
  } catch (error) {
