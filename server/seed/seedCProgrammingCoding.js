/**
 * seedCProgrammingCoding.js
 * Seeds CodingProblem documents for all C Programming for Problem Solving topics.
 * Run with: node server/seed/seedCProgrammingCoding.js
 */
...
const codingData = require("../data/subjects/cProgrammingCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find C Programming subject
  const subject = await Subject.findOne({ name: /c programming for problem solving/i });
  if (!subject) {
    console.error("C Programming for Problem Solving subject not found");
    process.exit(1);
  }
