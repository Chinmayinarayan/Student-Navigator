/**
 * seedComputerNetworksCoding.js
 * Seeds CodingProblem documents for all Computer Networks topics.
 * Run with: node server/seed/seedComputerNetworksCoding.js
 */
...
const codingData = require("../data/subjects/computerNetworksCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Computer Networks subject
  const subject = await Subject.findOne({ name: /^computer networks$/i });
  if (!subject) {
    console.error("Computer Networks subject not found");
    process.exit(1);
  }
