...
const updateOOPLabSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Object Oriented Programming Lab subject update...");

    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Object Oriented Programming Lab$/i }
    });
    ...
    if (!subjectDoc) {
      subjectDoc = await Subject.findOne({
        name: { $regex: /OOP Lab/i }
      });
    }

    if (!subjectDoc) {
      console.error("Subject 'Object Oriented Programming Lab' not found. Please create it first.");
      process.exit(1);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    const concepts = oopLabTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    console.log(`Updated subject.concepts with ${oopLabTopics.length} modules.`);
    ...
    const topicDocsToInsert = oopLabTopics.map(t => ({
    ...
    for (const rawTopic of oopLabTopics) {
    ...
    console.log("✅ Object Oriented Programming Lab update completed successfully!");
    process.exit(0);
  } catch (error) {
