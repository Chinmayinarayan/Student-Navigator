...
const updateMechanicalEngineeringSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Basic Mechanical Engineering subject update...");

    // 1. Find existing subject
    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Basic Mechanical Engineering$/i }
    });

    if (!subjectDoc) {
      console.log("Subject not found by exact match, searching by contains...");
      subjectDoc = await Subject.findOne({
        name: { $regex: /Basic Mechanical|Mechanical Engineering/i }
      });
    }
    ...
    if (!subjectDoc) {
      ...
      const subData = additionalSubjects.find(s => s.name === "Basic Mechanical Engineering") || {
        name: "Basic Mechanical Engineering",
        description: "Master fundamentals of mechanical engineering, engineering materials, mechanics force systems, friction, work-energy, simple machines, thermodynamics, heat transfer, IC engines, steam power plants, refrigeration, manufacturing processes, power transmission, robotics, CAD/CAM, and mechanical lab testing."
      };
    ...
    const concepts = mechanicalEngineeringTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    const topicDocsToInsert = mechanicalEngineeringTopics.map(t => ({
    ...
    for (const rawTopic of mechanicalEngineeringTopics) {
    ...
    console.log("✅ Basic Mechanical Engineering update completed successfully!");
    process.exit(0);
  } catch (error) {
...
updateMechanicalEngineeringSubject();
