/**
 * seedCloudComputingCoding.js
 * Seeds CodingProblem documents for all Cloud Computing topics.
 * Run with: node server/seed/seedCloudComputingCoding.js
 */
...
const codingData = require("../data/subjects/cloudComputingCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Cloud Computing subject
  const subject = await Subject.findOne({ name: /^cloud computing$/i });
  if (!subject) {
    console.error("Cloud Computing subject not found");
    process.exit(1);
  }
