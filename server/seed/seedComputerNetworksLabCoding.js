/**
 * seedComputerNetworksLabCoding.js
 * Seeds CodingProblem documents for all Computer Networks Lab topics.
 * Run with: node server/seed/seedComputerNetworksLabCoding.js
 */
...
const codingData = require("../data/subjects/computerNetworksLabCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Computer Networks Lab subject
  const subject = await Subject.findOne({ name: /^computer networks lab$/i });
  if (!subject) {
    console.error("Computer Networks Lab subject not found");
    process.exit(1);
  }
