...
const updateLinearAlgebraSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Linear Algebra subject update...");

    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Linear Algebra$/i }
    });
    ...
    if (!subjectDoc) {
      console.error("Subject 'Linear Algebra' not found. Please create it first.");
      process.exit(1);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    const concepts = linearAlgebraTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    console.log(`Updated subject.concepts with ${linearAlgebraTopics.length} modules.`);
    ...
    const topicDocsToInsert = linearAlgebraTopics.map(t => ({
    ...
    for (const rawTopic of linearAlgebraTopics) {
    ...
    console.log("✅ Linear Algebra update completed successfully!");
    process.exit(0);
  } catch (error) {
