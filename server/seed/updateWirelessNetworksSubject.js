...
const updateWirelessNetworksSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Wireless and Mobile Networks subject update...");

    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Wireless and Mobile Networks$/i }
    });

    if (!subjectDoc) {
      subjectDoc = await Subject.findOne({
        name: { $regex: /Wireless.*Mobile Networks/i }
      });
    }
    ...
    if (!subjectDoc) {
      console.error("Subject 'Wireless and Mobile Networks' not found. Please create it first.");
      process.exit(1);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    const concepts = wirelessNetworksTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    console.log(`Updated subject.concepts with ${wirelessNetworksTopics.length} modules.`);
    ...
    const topicDocsToInsert = wirelessNetworksTopics.map(t => ({
    ...
    for (const rawTopic of wirelessNetworksTopics) {
    ...
    console.log("✅ Wireless and Mobile Networks update completed successfully!");
    process.exit(0);
  } catch (error) {
