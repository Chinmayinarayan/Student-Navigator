/**
 * seedBasicMechanicalEngineeringCoding.js
 * Seeds CodingProblem documents for all Basic Mechanical Engineering topics.
 * Run with: node server/seed/seedBasicMechanicalEngineeringCoding.js
 */
...
const codingData = require("../data/subjects/basicMechanicalEngineeringCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Basic Mechanical Engineering subject
  const subject = await Subject.findOne({ name: /basic mechanical engineering/i });
  if (!subject) {
    console.error("Basic Mechanical Engineering subject not found");
    process.exit(1);
  }
