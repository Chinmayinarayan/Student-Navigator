/**
 * seedLinearAlgebraCoding.js
 * Seeds CodingProblem documents for all Linear Algebra topics.
 * Run with: node server/seed/seedLinearAlgebraCoding.js
 */
...
const codingData = require("../data/subjects/linearAlgebraCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Linear Algebra subject
  const subject = await Subject.findOne({ name: /^linear algebra$/i });
  if (!subject) {
    console.error("Linear Algebra subject not found");
    process.exit(1);
  }
  ...
  for (const entry of codingData) {
    ...
    for (let i = 0; i < entry.recommendedChallenges.length; i++) {
      ...
      await CodingProblem.create({
        topic: topic._id,
        title: ch.title,
        description: `Practice exercise: ${ch.title}. Solve on ${ch.platform}.`,
