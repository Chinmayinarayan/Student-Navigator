...
console.log('=== RUNNING ACCURATE & VERIFIED YOUTUBE RESOURCES UPDATE ===');

// 100% verified, subject-specific, working YouTube links (no duplicate/mismatched links)
const accuratePlaylists = {
...
// 1. Update Static Files on Disk
const subjectsDir = path.join(__dirname, '../data/subjects');
console.log(`\n--- UPDATING STATIC FILES IN ${subjectsDir} ---`);
  ...
  if (file.endsWith('.js') && !file.includes('Topics') && file !== 'index.js' && file !== 'additionalSubjects.js') {
    ...
    try {
      ...
      if (subject && subject.name) {
        const newYt = accuratePlaylists[subject.name];
        if (newYt) {
          console.log(`Updating static file: "${file}" (${subject.name}) -> ${newYt.length} youtubeResources`);
...
// Update additionalSubjects.js
const additionalPath = path.join(__dirname, '../data/additionalSubjects.js');
console.log(`\n--- UPDATING STATIC FILE additionalSubjects.js ---`);
  ...
  try {
    ...
    if (Array.isArray(subjects)) {
      subjects.forEach(subject => {
        const newYt = accuratePlaylists[subject.name];
        if (newYt) {
          console.log(`Updating subject in additionalSubjects: "${subject.name}" -> ${newYt.length} youtubeResources`);
          subject.youtubeResources = newYt;
        }
      });
...
(async () => {
  try {
    ...
    console.log(`Found ${allSubjects.length} subjects in MongoDB database.`);

    let dbUpdated = 0;
    for (let sub of allSubjects) {
      const newYt = accuratePlaylists[sub.name];
      if (newYt) {
        sub.youtubeResources = newYt;
        sub.markModified("youtubeResources");
        await sub.save();
        dbUpdated++;
        console.log(`✅ Updated youtubeResources in MongoDB for "${sub.name}" (${newYt.length} links).`);
      } else {
        console.log(`⚠️ No playlist mapping found for subject "${sub.name}".`);
      }
    }

    console.log(`\n🎉 Success! Rebuilt and updated accurate YouTube links for all subjects.`);
    console.log(`Static files updated: ${filesUpdated + 1}`);
