/**
 * seedPhysicsLabCoding.js
 * Seeds CodingProblem documents for all Applied Physics Lab topics.
 * Run with: node server/seed/seedPhysicsLabCoding.js
 */
...
const codingData = require("../data/subjects/appliedPhysicsCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Applied Physics Lab subject
  const subject = await Subject.findOne({ name: /applied physics lab/i });
  if (!subject) {
    console.error("Applied Physics Lab subject not found");
    process.exit(1);
  }
  ...
  for (const entry of codingData) {
    ...
    // Remove any obsolete problems for this topic
    const validTitles = entry.recommendedChallenges.map((c) => c.title.toLowerCase());
    await CodingProblem.deleteMany({
      topic: topic._id,
      title: { $nin: entry.recommendedChallenges.map((c) => c.title) }
    });
