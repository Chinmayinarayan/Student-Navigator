/**
 * seedBigDataCoding.js
 * Seeds CodingProblem documents for all Big Data Analytics topics.
 * Run with: node server/seed/seedBigDataCoding.js
 */
...
const codingData = require("../data/subjects/bigDataCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Big Data Analytics subject
  const subject = await Subject.findOne({ name: /big data analytics/i });
  if (!subject) {
    console.error("Big Data Analytics subject not found");
    process.exit(1);
  }
