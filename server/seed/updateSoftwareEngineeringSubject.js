...
const updateSoftwareEngineeringSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Software Engineering subject update...");

    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Software Engineering$/i }
    });
    ...
    if (!subjectDoc) {
      console.error("Subject 'Software Engineering' not found. Please create it first.");
      process.exit(1);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    const concepts = softwareEngineeringTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    console.log(`Updated subject.concepts with ${softwareEngineeringTopics.length} modules.`);
    ...
    const topicDocsToInsert = softwareEngineeringTopics.map(t => ({
    ...
    for (const rawTopic of softwareEngineeringTopics) {
    ...
    console.log("✅ Software Engineering update completed successfully!");
    process.exit(0);
  } catch (error) {
...
updateSoftwareEngineeringSubject();
