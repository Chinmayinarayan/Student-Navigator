/**
 * seedGenerativeAiCoding.js
 * Seeds CodingProblem documents for all Generative AI topics.
 * Run with: node server/seed/seedGenerativeAiCoding.js
 */
...
const codingData = require("../data/subjects/generativeAiCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Generative AI subject
  const subject = await Subject.findOne({ name: /^generative ai$/i });
  if (!subject) {
    console.error("Generative AI subject not found");
    process.exit(1);
  }
  ...
  for (const entry of codingData) {
    ...
    for (let i = 0; i < entry.recommendedChallenges.length; i++) {
      ...
      await CodingProblem.create({
        topic: topic._id,
        title: ch.title,
        description: `Practice problem / tutorial: ${ch.title}. Practice on ${ch.platform}.`,
