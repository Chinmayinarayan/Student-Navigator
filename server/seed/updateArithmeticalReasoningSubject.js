...
const updateArithmeticalReasoningSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Arithmetical Thinking & Analytical Reasoning subject update...");

    // 1. Find existing subject
    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Arithmetical Thinking & Analytical Reasoning$/i }
    });
    ...
    if (!subjectDoc) {
      ...
      const subData = additionalSubjects.find(s => s.name === "Arithmetical Thinking & Analytical Reasoning") || {
        name: "Arithmetical Thinking & Analytical Reasoning",
        description: "Master quantitative aptitude, number systems, arithmetic operations, ratio-proportion, percentage, time-work, TSD, interest, logical reasoning, data interpretation, and placement exam patterns."
      };
    ...
    // 2. Update subject's concepts list to match the 14 modules
    const concepts = arithmeticalReasoningTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    console.log("Updated subject.concepts with 14 modules.");
    ...
    const topicDocsToInsert = arithmeticalReasoningTopics.map(t => ({
    ...
    for (const rawTopic of arithmeticalReasoningTopics) {
    ...
    console.log("✅ Arithmetical Thinking & Analytical Reasoning update completed successfully!");
    process.exit(0);
  } catch (error) {
...
updateArithmeticalReasoningSubject();
