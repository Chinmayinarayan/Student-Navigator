/**
 * seedDbmsLabCoding.js
 * Seeds CodingProblem documents for all Database Applications Lab topics.
 * Run with: node server/seed/seedDbmsLabCoding.js
 */
...
const codingData = require("../data/subjects/dbmsLabCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Database Applications Lab subject
  const subject = await Subject.findOne({ name: /database applications lab/i });
  if (!subject) {
    console.error("Database Applications Lab subject not found");
    process.exit(1);
  }
