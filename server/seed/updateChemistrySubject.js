...
const updateChemistrySubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Engineering Chemistry subject update...");

    // 1. Find existing subject
    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Engineering Chemistry$/i }
    });
    ...
    if (!subjectDoc) {
      ...
      const chemistryData = additionalSubjects.find(s => s.name.toLowerCase().includes("engineering chemistry"));
      subjectDoc = await Subject.create(chemistryData);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    // 2. Update subject's concepts list to match the 13 modules
    const concepts = chemistryTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    const topicDocsToInsert = chemistryTopics.map(t => ({
    ...
    for (const rawTopic of chemistryTopics) {
    ...
    console.log("✅ Engineering Chemistry update completed successfully!");
    process.exit(0);
  } catch (error) {
