/**
 * seedDbmsCoding.js
 * Seeds CodingProblem documents for all Database Management System topics.
 * Run with: node server/seed/seedDbmsCoding.js
 */
...
const codingData = require("../data/subjects/dbmsCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Database Management System subject
  const subject = await Subject.findOne({ name: /^database management system$/i });
  if (!subject) {
    console.error("Database Management System subject not found");
    process.exit(1);
  }
