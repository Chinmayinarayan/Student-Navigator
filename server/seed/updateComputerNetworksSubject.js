...
const updateComputerNetworksSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Computer Networks subject update...");

    // 1. Find existing subject
    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Computer Networks$/i }
    });
    ...
    if (!subjectDoc) {
      ...
      const networksData = require("../data/subjects/networks");
      subjectDoc = await Subject.create(networksData);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    // 2. Update subject's concepts list to match the 12 modules
    const concepts = computerNetworksTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    console.log("Updated subject.concepts with 12 modules.");
    ...
    const topicDocsToInsert = computerNetworksTopics.map(t => ({
    ...
    for (const rawTopic of computerNetworksTopics) {
    ...
    console.log("✅ Computer Networks update completed successfully!");
    process.exit(0);
  } catch (error) {
