/**
 * seedBasicElectricalEngineeringCoding.js
 * Seeds CodingProblem documents for all Basic Electrical Engineering topics.
 * Run with: node server/seed/seedBasicElectricalEngineeringCoding.js
 */
...
const codingData = require("../data/subjects/basicElectricalEngineeringCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Basic Electrical Engineering subject
  const subject = await Subject.findOne({ name: /basic electrical engineering/i });
  if (!subject) {
    console.error("Basic Electrical Engineering subject not found");
    process.exit(1);
  }
