/**
 * seedBasicElectronicsCoding.js
 * Seeds CodingProblem documents for all Basic Electronics topics.
 * Run with: node server/seed/seedBasicElectronicsCoding.js
 */
...
const codingData = require("../data/subjects/basicElectronicsCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Basic Electronics subject
  const subject = await Subject.findOne({ name: /basic electronics/i });
  if (!subject) {
    console.error("Basic Electronics subject not found");
    process.exit(1);
  }
