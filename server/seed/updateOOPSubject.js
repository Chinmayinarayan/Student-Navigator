...
const updateOOPSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Object Oriented Programming subject update...");

    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Object Oriented Programming$/i }
    });
    ...
    if (!subjectDoc) {
      console.error("Subject 'Object Oriented Programming' not found. Please create it first.");
      process.exit(1);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    const concepts = oopTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    console.log(`Updated subject.concepts with ${oopTopics.length} modules.`);
    ...
    const topicDocsToInsert = oopTopics.map(t => ({
    ...
    for (const rawTopic of oopTopics) {
    ...
    console.log("✅ Object Oriented Programming update completed successfully!");
    process.exit(0);
  } catch (error) {
