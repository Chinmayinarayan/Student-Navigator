...
const updateCProgrammingSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for C Programming for Problem Solving subject update...");

    // 1. Find existing subject
    let subjectDoc = await Subject.findOne({
      name: { $regex: /^C Programming for Problem Solving$/i }
    });

    if (!subjectDoc) {
      console.log("Subject not found by exact match, searching by contains...");
      subjectDoc = await Subject.findOne({
        name: { $regex: /C Programming/i }
      });
    }
    ...
    if (!subjectDoc) {
      ...
      const subData = additionalSubjects.find(s => s.name === "C Programming for Problem Solving") || {
        name: "C Programming for Problem Solving",
        description: "Master C programming from problem-solving flowcharts, tokens, variables, data types, operators, decision making, looping, functions, arrays, strings, pointers, structures, unions, file handling, dynamic memory allocation (malloc/calloc), data structures implementation, basic searching/sorting algorithms, and mini-projects."
      };
    ...
    const concepts = cProgrammingTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    const topicDocsToInsert = cProgrammingTopics.map(t => ({
    ...
    for (const rawTopic of cProgrammingTopics) {
    ...
    console.log("✅ C Programming for Problem Solving update completed successfully!");
    process.exit(0);
  } catch (error) {
