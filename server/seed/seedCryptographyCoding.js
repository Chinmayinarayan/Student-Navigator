/**
 * seedCryptographyCoding.js
 * Seeds CodingProblem documents for all Cryptography & Network Security topics.
 * Run with: node server/seed/seedCryptographyCoding.js
 */
...
const codingData = require("../data/subjects/cryptographyCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Cryptography & Network Security subject
  const subject = await Subject.findOne({ name: /cryptography/i });
  if (!subject) {
    console.error("Cryptography & Network Security subject not found");
    process.exit(1);
  }
