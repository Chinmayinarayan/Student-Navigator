...
const updateDiscreteSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Discrete Mathematics subject update...");

    // 1. Find existing subject
    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Discrete Mathematics$/i }
    });
    ...
    if (!subjectDoc) {
      ...
      const discreteData = require("../data/subjects/discrete");
      subjectDoc = await Subject.create(discreteData);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    // 2. Update subject's concepts list to match the 15 modules
    const concepts = discreteTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    const topicDocsToInsert = discreteTopics.map(t => ({
    ...
    for (const rawTopic of discreteTopics) {
    ...
    console.log("✅ Discrete Mathematics update completed successfully!");
    process.exit(0);
  } catch (error) {
