...
const updateOSSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Operating System update...");
    ...
    if (!subjectDoc) {
      console.error("Subject 'Operating System Principles and Programming' not found. Please create it first.");
      process.exit(1);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    // 2. Update subject's concepts list
    const concepts = osTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    console.log(`Updated subject.concepts with ${osTopics.length} modules.`);

    // 3. Remove existing topics for this subject
    ...
    const topicDocsToInsert = osTopics.map(t => ({
    ...
    // Map title -> _id for prerequisites
    const titleToIdMap = {};
    ...
    for (const rawTopic of osTopics) {
    ...
    console.log("✅ Operating System Principles and Programming update completed successfully!");
    process.exit(0);
  } catch (error) {
