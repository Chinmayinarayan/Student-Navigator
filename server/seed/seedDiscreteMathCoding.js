/**
 * seedDiscreteMathCoding.js
 * Seeds CodingProblem documents for all Discrete Mathematics topics.
 * Run with: node server/seed/seedDiscreteMathCoding.js
 */
...
const codingData = require("../data/subjects/discreteMathCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Discrete Mathematics subject
  const subject = await Subject.findOne({ name: /^discrete mathematics$/i });
  if (!subject) {
    console.error("Discrete Mathematics subject not found");
    process.exit(1);
  }
