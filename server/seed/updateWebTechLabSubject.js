...
const updateWebTechLabSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Web Technologies Lab subject update...");

    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Web Technologies Lab$/i }
    });
    ...
    if (!subjectDoc) {
      subjectDoc = await Subject.findOne({
        name: { $regex: /Web Tech/i }
      });
    }

    if (!subjectDoc) {
      console.error("Subject 'Web Technologies Lab' not found. Please create it first.");
      process.exit(1);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    const concepts = webTechLabTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    console.log(`Updated subject.concepts with ${webTechLabTopics.length} modules.`);
    ...
    const topicDocsToInsert = webTechLabTopics.map(t => ({
    ...
    for (const rawTopic of webTechLabTopics) {
    ...
    console.log("✅ Web Technologies Lab update completed successfully!");
    process.exit(0);
  } catch (error) {
