...
const updateAimlSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Artificial Intelligence and Machine Learning subject update...");

    // 1. Find existing subject
    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Artificial Intelligence and Machine Learning$/i }
    });

    if (!subjectDoc) {
      console.log("Subject not found by exact match, searching by contains...");
      subjectDoc = await Subject.findOne({
        name: { $regex: /Artificial Intelligence|Machine Learning/i }
      });
    }

    if (!subjectDoc) {
      console.log("Creating new subject as fallback...");
      const aimlData = require("../data/subjects/aiml");
      subjectDoc = await Subject.create(aimlData);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    // 2. Update subject's concepts list to match the 16 modules
    const concepts = aimlTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    const topicDocsToInsert = aimlTopics.map(t => ({
    ...
    for (const rawTopic of aimlTopics) {
    ...
    console.log("✅ Artificial Intelligence and Machine Learning update completed successfully!");
    process.exit(0);
  } catch (error) {
