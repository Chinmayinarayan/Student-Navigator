...
const updateProbabilityStatsSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Probability and Statistics subject update...");
    ...
    if (!subjectDoc) {
      console.error("Subject 'Probability and Statistics' not found. Please create it first.");
      process.exit(1);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    const concepts = probabilityStatsTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    console.log(`Updated subject.concepts with ${probabilityStatsTopics.length} modules.`);
    ...
    const topicDocsToInsert = probabilityStatsTopics.map(t => ({
    ...
    for (const rawTopic of probabilityStatsTopics) {
    ...
    console.log("✅ Probability and Statistics update completed successfully!");
    process.exit(0);
  } catch (error) {
