/**
 * seedArithmeticalReasoningCoding.js
 * Seeds CodingProblem documents for all Arithmetical Thinking & Analytical Reasoning topics.
 * Run with: node server/seed/seedArithmeticalReasoningCoding.js
 */
...
const codingData = require("../data/subjects/arithmeticalReasoningCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Arithmetical Thinking subject
  const subject = await Subject.findOne({ name: /arithmetical thinking/i });
  if (!subject) {
    console.error("Arithmetical Thinking & Analytical Reasoning subject not found");
    process.exit(1);
  }
