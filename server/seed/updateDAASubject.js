...
const updateDAASubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Design and Analysis of Algorithms subject update...");

    // 1. Find existing subject
    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Design and Analysis of Algorithms$/i }
    });
    ...
    const topicDocsToInsert = daaTopics.map(t => ({
    ...
    for (const rawTopic of daaTopics) {
    ...
    console.log("✅ Design and Analysis of Algorithms update completed successfully!");
    process.exit(0);
  } catch (error) {
