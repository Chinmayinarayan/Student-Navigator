/**
 * seedEngineeringPhysicsCoding.js
 * Seeds CodingProblem documents for all Engineering Physics topics.
 * Run with: node server/seed/seedEngineeringPhysicsCoding.js
 */
...
const codingData = require("../data/subjects/engineeringPhysicsCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Engineering Physics subject
  const subject = await Subject.findOne({ name: /^engineering physics$/i });
  if (!subject) {
    console.error("Engineering Physics subject not found");
    process.exit(1);
  }
