...
const updateCompilerDesignSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Compiler Design subject update...");

    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Principles of Compiler Design$/i }
    });
    ...
    if (!subjectDoc) {
      console.error("Subject 'Principles of Compiler Design' not found. Please create it first.");
      process.exit(1);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    const concepts = compilerDesignTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    console.log(`Updated subject.concepts with ${compilerDesignTopics.length} modules.`);
    ...
    const topicDocsToInsert = compilerDesignTopics.map(t => ({
    ...
    for (const rawTopic of compilerDesignTopics) {
    ...
    console.log("✅ Principles of Compiler Design update completed successfully!");
    process.exit(0);
  } catch (error) {
