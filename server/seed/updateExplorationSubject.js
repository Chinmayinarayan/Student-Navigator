...
const updateExplorationSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Engineering Exploration subject update...");

    // 1. Find existing subject
    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Engineering Exploration$/i }
    });
    ...
    if (!subjectDoc) {
      ...
      const explorationData = additionalSubjects.find(s => s.name.toLowerCase().includes("engineering exploration"));
      subjectDoc = await Subject.create(explorationData);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    // 2. Update subject's concepts list to match the 15 modules
    const concepts = engineeringExplorationTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    const topicDocsToInsert = engineeringExplorationTopics.map(t => ({
    ...
    for (const rawTopic of engineeringExplorationTopics) {
    ...
    console.log("✅ Engineering Exploration update completed successfully!");
    process.exit(0);
  } catch (error) {
