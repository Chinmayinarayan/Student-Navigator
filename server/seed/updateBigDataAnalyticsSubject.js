...
const updateBigDataAnalyticsSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Big Data Analytics subject update...");

    // 1. Find existing subject
    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Big Data Analytics$/i }
    });

    if (!subjectDoc) {
      console.log("Subject not found by exact match, searching by contains...");
      subjectDoc = await Subject.findOne({
        name: { $regex: /Big Data Analytics|Big Data/i }
      });
    }
    ...
    if (!subjectDoc) {
      ...
      const subData = additionalSubjects.find(s => s.name === "Big Data Analytics") || {
        name: "Big Data Analytics",
        description: "Master Big Data fundamentals, 5 V's characteristics, Lambda/Kappa architectures, HDFS distributed storage, Hadoop YARN, MapReduce programming, Apache Spark (RDDs, DataFrames, MLlib), Hive, Pig, Sqoop, NoSQL (MongoDB, Cassandra), Data Mining (Apriori), PySpark ML, Tableau/Power BI visualization, Apache Kafka real-time streaming, Cloud platforms (AWS/GCP), security (Kerberos/Ranger), and industrial projects."
      };
    ...
    const concepts = bigDataAnalyticsTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    const topicDocsToInsert = bigDataAnalyticsTopics.map(t => ({
    ...
    for (const rawTopic of bigDataAnalyticsTopics) {
    ...
    console.log("✅ Big Data Analytics update completed successfully!");
    process.exit(0);
  } catch (error) {
