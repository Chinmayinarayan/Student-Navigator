/**
 * seedEdaCoding.js
 * Seeds CodingProblem documents for all Exploratory Data Analysis topics.
 * Run with: node server/seed/seedEdaCoding.js
 */
...
const codingData = require("../data/subjects/edaCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Exploratory Data Analysis subject
  const subject = await Subject.findOne({ name: /exploratory data analysis/i });
  if (!subject) {
    console.error("Exploratory Data Analysis subject not found");
    process.exit(1);
  }
