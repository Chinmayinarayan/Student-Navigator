...
const updateDSASubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Data Structures and Algorithms subject update...");

    // 1. Find existing subject
    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Data Structures and Algorithms$/i }
    });
    ...
    if (!subjectDoc) {
      ...
      const dsaData = require("../data/subjects/dsa");
      subjectDoc = await Subject.create(dsaData);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    // 2. Update subject's concepts list to match the 20 modules
    const concepts = dsaTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    console.log("Updated subject.concepts with 20 modules.");
    ...
    const topicDocsToInsert = dsaTopics.map(t => ({
    ...
    for (const rawTopic of dsaTopics) {
    ...
    console.log("✅ Data Structures and Algorithms update completed successfully!");
    process.exit(0);
  } catch (error) {
