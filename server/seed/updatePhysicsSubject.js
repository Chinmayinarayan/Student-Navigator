...
const updatePhysicsSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Engineering Physics subject update...");

    // 1. Find existing subject
    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Engineering Physics$/i }
    });
    ...
    if (!subjectDoc) {
      ...
      const physics = require("../data/subjects/physics");
      const physicsData = physics.find(s => s.name.toLowerCase().includes("engineering physics"));
      subjectDoc = await Subject.create(physicsData);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    // 2. Update subject's concepts list to match the 15 modules
    const concepts = engineeringPhysicsTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    const topicDocsToInsert = engineeringPhysicsTopics.map(t => ({
    ...
    for (const rawTopic of engineeringPhysicsTopics) {
    ...
    console.log("✅ Engineering Physics update completed successfully!");
    process.exit(0);
  } catch (error) {
