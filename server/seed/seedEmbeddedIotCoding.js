/**
 * seedEmbeddedIotCoding.js
 * Seeds CodingProblem documents for all Embedded Systems and IoT topics.
 * Run with: node server/seed/seedEmbeddedIotCoding.js
 */
...
const codingData = require("../data/subjects/embeddedIotCodingPractice");

async function seed() {
  await mongoose.connect(process.env.MONGODB_URI);
  console.log("Connected to MongoDB");

  // Find Embedded Systems and IoT subject
  const subject = await Subject.findOne({ name: /embedded systems and iot/i });
  if (!subject) {
    console.error("Embedded Systems and IoT subject not found");
    process.exit(1);
  }
  ...
  for (const entry of codingData) {
    const topic = await Topic.findOne({
      subject: subject._id,
      title: new RegExp(`^${entry.topic.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&")}$`, "i")
    });
    if (!topic) {
    ...
    for (let i = 0; i < entry.recommendedChallenges.length; i++) {
      ...
      await CodingProblem.create({
        topic: topic._id,
        title: ch.title,
        description: `Practice problem: ${ch.title}. Practice on ${ch.platform}.`,
