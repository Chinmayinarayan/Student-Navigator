...
const updateMultivariableCalculusSubject = async () => {
  try {
    await connectDB();
    console.log("Connected to database for Multivariable Calculus subject update...");

    let subjectDoc = await Subject.findOne({
      name: { $regex: /^Multivariable Calculus$/i }
    });
    ...
    if (!subjectDoc) {
      console.error("Subject 'Multivariable Calculus' not found. Please create it first.");
      process.exit(1);
    }

    console.log(`Found subject: "${subjectDoc.name}" (${subjectDoc._id})`);

    const concepts = multivariableCalculusTopics.map(t => ({
      title: t.title,
      topics: t.subTopics || []
    }));
    ...
    console.log(`Updated subject.concepts with ${multivariableCalculusTopics.length} modules.`);
    ...
    const topicDocsToInsert = multivariableCalculusTopics.map(t => ({
    ...
    for (const rawTopic of multivariableCalculusTopics) {
    ...
    console.log("✅ Multivariable Calculus update completed successfully!");
    process.exit(0);
  } catch (error) {
...
updateMultivariableCalculusSubject();
